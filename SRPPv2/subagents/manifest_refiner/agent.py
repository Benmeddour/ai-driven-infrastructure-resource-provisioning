...
manifest_refiner = LlmAgent(
    ...
    **Current tf:**
    ...
    take this tf file as an example of a correct tf file, there are some (costants) that should not be changed:
    # Terraform configuration block
        ...
        # cicustom = var.cicustom (costants)

        # Whether to automatically upgrade cloud-init tools on first boot
        ciupgrade   = true (costants)

        # DNS servers for the VM to use (costants)
        nameserver  = "192.168.16.2 8.8.8.8"

        # Static IP configuration for the first interface
        ipconfig0   = "ip=192.168.0.5/16,gw=192.168.0.1"

        # Public SSH key to inject into the VM (for key-based login) (costants)
        sshkeys     = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIP8mGrP9ceX0JmrTXkVp7x/nsaLiyxPF68paem0zOu55 admin-ubt"

        # Default cloud-init username (costants)
        ciuser      = "admin-ubt"

        # Default cloud-init password (costants)
        cipassword  = "admin"
      }
    IMPORTANT: Do use this tf file as a template, the tf file should be an exact match for this.
